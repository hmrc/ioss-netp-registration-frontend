...
class CheckIntermediaryAccessFilterSpec extends SpecBase with MockitoSugar with EtmpModelGenerators {
  ...
  ".filter" - {
    ...
    "must redirect to AccessDenied page" - {
      ...
      "when there are multiple previous registrations but none of them have access to an IOSS client" in {

        val multipleEnrolments = Enrolments(
          Set(
            Enrolment("HMRC-IOSS-INT", Seq(EnrolmentIdentifier("HMRC-IOSS-INT", "IN900111111")), "Activated"),
            Enrolment("HMRC-IOSS-INT", Seq(EnrolmentIdentifier("HMRC-IOSS-INT", "IN900111112")), "Activated")
          )
        )
