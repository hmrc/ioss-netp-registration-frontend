...
import pages.{ClientVatNumberPage, EmptyWaypoints, Waypoints}
...
class ClientVatNumberControllerSpec extends SpecBase with MockitoSugar {

  private val waypoints: Waypoints = EmptyWaypoints
  private val ukVatNumber: String = "GB123456789"

  val formProvider = new ClientVatNumberFormProvider()
  val form: Form[String] = formProvider()

  lazy val clientVatNumberRoute: String = routes.ClientVatNumberController.onPageLoad(waypoints).url
  ...
  "ClientVatNumber Controller" - {
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result = route(application, request).value
        val expectedAnswers = emptyUserAnswers.set(ClientVatNumberPage, ukVatNumber).success.value

        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual ClientVatNumberPage.navigate(waypoints, emptyUserAnswers, expectedAnswers).url
      }
    }
