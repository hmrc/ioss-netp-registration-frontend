...
class PreviousEuCountryControllerSpec extends SpecBase with MockitoSugar {
  ...
  "PreviousEuCountry Controller" - {
    ...
    "must save the answer and redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request = FakeRequest(POST, previousEuCountryRoute)
            .withFormUrlEncodedBody(("value", country.code))

        val result = route(application, request).value
        val expectedAnswers = emptyUserAnswersWithVatInfo.set(PreviousEuCountryPage(index), country).success.value
