...
class PreviousOssNumberControllerSpec extends SpecBase with MockitoSugar {
  ...
  val formProvider = new PreviousOssNumberFormProvider()
  val form = formProvider()

  lazy val previousOssNumberRoute = routes.PreviousOssNumberController.onPageLoad(NormalMode).url
  ...
  "PreviousOssNumber Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(PreviousOssNumberPage, "answer").success.value
