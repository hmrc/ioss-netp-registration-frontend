...
class CheckVatDetailsSpec extends AnyFreeSpec with Matchers with ScalaCheckPropertyChecks with OptionValues {

  "CheckVatDetails" - {

    "must deserialise valid values" in {

      val gen = Gen.oneOf(CheckVatDetails.values)
    ...
    "must fail to deserialise invalid values" in {
      ...
      val gen = arbitrary[String] suchThat (!CheckVatDetails.values.map(_.toString).contains(_))
